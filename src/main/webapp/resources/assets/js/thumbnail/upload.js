/**
 * thumbnail 이미지 관련 자바스크립트
 */
function initialize() {
    $("#btnDelete").hide(); //처음 구동시 버튼 숨김.
    // 구분 변경시
...
//          prdNicNmNm.value = $("#nicNm").val();
    ...
    $("#btnEnrolment").on("click", function() {
        parent.co.popup.open($(document), "프로모션 등록", "/thumbnailUploadReg.do", 1000, 600);
    });
    ...
    $("a img[name='btnModify']").on("click", function() {
        ...
        parent.co.popup.open($(document), "프로모션 수정", "/thumbnailUploadMod.do" + params, 800, 150);
    });
    ...
    $("#btnDelete").on("click", function() {
        with (frmImage) {
            ...
            if (confirm("선택한 자료를 삭제하시겠습니까?")) {
                ...
                requestAjax("/thumbnailUploadDelete.do", params, callbackDelete);
            }
        }
    });
...
function popupNic() {
    if ($("input[type='radio'][name='imgCf']:checked").val() == "2") {
        ...
        parent.co.popup.open($(document), "카드할인 검색", "/commonCardList.do?allowAll=Y", 500, 530);
    }
    else {
        // 닉네임 검색
        if ($("#itemCd").val() == "") {
...
function previewImage(path, name) {
    ...
    parent.co.popup.open($(document), "이미지 미리보기", "/thumbnailUploadPreview.do" + param, 0, 0);
}
